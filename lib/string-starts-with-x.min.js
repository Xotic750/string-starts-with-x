 ...
 */
"use strict";var isWorking,nativeStartsWith="function"==typeof String.prototype.startsWith&&String.prototype.startsWith;if(nativeStartsWith){var attempt=_dereq_("attempt-x"),res=attempt.call("/a/",nativeStartsWith,/a/);(isWorking=res.threw)&&(isWorking=!1===(res=attempt.call("abc",nativeStartsWith,"a",Infinity)).threw&&!1===res.value),isWorking&&(isWorking=!1===(res=attempt.call(123,nativeStartsWith,"1")).threw&&!0===res.value),isWorking&&(isWorking=(res=attempt.call(null,nativeStartsWith,"n")).threw)}var $startsWith;if(isWorking)$startsWith=function _startsWith(string,searchString){var args=[searchString];return arguments.length>2&&(args[1]=arguments[2]),nativeStartsWith.apply(string,args)};else{var toInteger=_dereq_("to-integer-x"),requireObjectCoercible=_dereq_("require-object-coercible-x"),toStr=_dereq_("to-string-x"),isRegExp=_dereq_("is-regexp-x");$startsWith=function _startsWith(string,searchString){var str=toStr(requireObjectCoercible(string));if(isRegExp(searchString))throw new TypeError('Cannot call method "startsWith" with a regex');var searchStr=toStr(searchString),position=arguments.length>2?toInteger(arguments[2]):0,start=position>0?position:0;return str.slice(start,start+searchStr.length)===searchStr}}module.exports=$startsWith},{"attempt-x":4,"is-regexp-x":18,"require-object-coercible-x":30,"to-integer-x":33,"to-string-x":40}],2:[function(_dereq_,module,exports){/**
